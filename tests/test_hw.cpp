...
TEST_F(test_Sorting, TestMerge) {
  // This test is ungraded, but it might help you with the mergesort algo
  vector<int> left{3, 5, 8, 9, 11};
  vector<int> right{2, 4, 5, 8, 10, 11, 12};
  vector<int> result;
  merge(left, right, result);
  vector<int> expectedResult{2, 3, 4, 5, 5, 8, 8, 9, 10, 11, 11, 12};
  EXPECT_TRUE(vectors_contain_same(result, expectedResult));
}
...
bool is_unprocessed(vector<int>& data, vector<int>& result) {
  ...
  if (data.size() != result.size())
    return false;
  if (data.size() == 0)
    return false; // they are both empty, this is okay
  ...
  if (data.size() == 1)
    return false; // only one item, they are same, this is okay
